...
void move2(double dist, double (*correctionFunc)() = []()->double{return 0;}) {
  // Reset encoder positions
  // leftEncoder.resetRotation();
  // rightEncoder.resetRotation();
  resetMotorEncoders();
...
void autonomous(void) {
 ...
 frontClamp.set(true);
 wait(12, msec);
 Drivetrain.driveFor(reverse, 40, inches, 600, velocityUnits::rpm);
 frontClamp.set(false);
 goalCover.set(false);
 Drivetrain.driveFor(reverse, 25, inches, 300, velocityUnits::rpm);
 Drivetrain.turnToHeading(323, degrees);
 move(50);
 frontClamp.set(true);
 Drivetrain.driveFor(reverse, 55, inches, 600, velocityUnits::rpm);
